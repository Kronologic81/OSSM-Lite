...
namespace homing {
    void clearHoming() {
        ...
        //Add the pin initialization at the start of this function:
    ...
    static void startHomingTask(void *pvParameters) {
        ...
        while (isInCorrectState()) {
            ...
            // --- CHANGED LOGIC START ---
            // Replaced getAnalogAveragePercent with digitalRead for the NPN sensor.
            // Assuming INPUT_PULLUP was set in clearHoming(), the pin is HIGH normally,
            // and goes LOW when metal is detected by the NPN sensor.
            
            if (digitalRead(Pins::Driver::limitSwitchPin) == HIGH) {
                vTaskDelay(1);
                continue;
            }

            ESP_LOGD("Homing", "Limit switch triggered (Pin LOW)");
            // --- CHANGED LOGIC END ---
            stepper->stopMove();
